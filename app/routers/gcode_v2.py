"""Versioned v2 API and WebSocket endpoints for G-code optimization.

Provides the same functionality as the legacy endpoints but with typed
request/response schemas and a ``/api/v2`` prefix.  Legacy routes in
``routers/gcode.py`` remain untouched for backward compatibility.
"""
...
def _serialize_paths(paths):
    return [[{"x": p[0], "y": p[1]} for p in path.points] for path in paths]
...
async def upload_file(
    ...
    parsed_settings = OptimizerSettings(**{**OptimizerSettings().model_dump(), **user_settings})

    opt = GcodeOptimizer()
    paths = opt.parse(gcode_text)
    _apply_settings(opt, user_settings)

    serializable_paths = _serialize_paths(paths)
...
@router.post("/upload-svg", response_model=UploadResponse)
async def upload_svg(
    ...
    if not paths:
        raise HTTPException(
            status_code=422,
            detail="No drawable paths found in SVG.",
        )
...
async def get_job(job_id: str):
    ...
    job = jobs[job_id]
    settings = OptimizerSettings(**{**OptimizerSettings().model_dump(), **job.get("settings", {})})
    return JobStatusResponse(
        job_id=job_id,
        status=job["status"],
        path_count=len(job["paths"]),
        settings=settings,
    )
...
async def download(job_id: str):
    ...
    if job_id not in jobs:
        raise HTTPException(status_code=404, detail="Job not found")
    job = jobs[job_id]
    if not job.get("optimized_gcode"):
...
async def get_settings_schema():
    """Return the JSON Schema for optimizer settings."""
    return OptimizerSettings.model_json_schema()
...
async def websocket_endpoint(websocket: WebSocket, job_id: str):
    ...
    greedy_paths_data = _serialize_paths(greedy_paths)
    ...
    if merge_threshold > 0:
        ...
        if merge_stats["merge_count"] > 0:
            ...
            merged_paths_data = _serialize_paths(merged_paths)
    ...
    paths_data = _serialize_paths(optimized_paths)
